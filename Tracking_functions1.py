...
def calculate_distance(cx, cy, cxpts, cypts, dist_travelled, large_movement_threshold):
    ...
    """
    inclusion_status = "Unknown"  #Give inclusion status a defintion before if block
    ...
    cumul_dist_included = 0
    #sum each value found in the dist_travelled list
    for cumul_dist_travelled in range(1, len(dist_travelled)):
        cumul_dist_travelled = sum(dist_travelled) # Update cumulative distance regardless of inclusion status
   
    if Pixel_dist < large_movement_threshold:
       inclusion_status = "Included"
    else:
       inclusion_status = "Excluded"  # Frame distance is too large

     # Calculate cumulative distance for included frames
    if inclusion_status == "Included":
       cumul_dist_included = sum([d for d in dist_travelled if d < large_movement_threshold])
    

    #print(f"Pixel_dist: {Pixel_dist}")
    #print(f"Inclusion Status: {inclusion_status}")
    #print(f"Cumulative Distance Included: {cumul_dist_included}") #removed for now
    ...
    cumul_dist_included = round(cumul_dist_included, 2)

    return ix, iy, Pixel_dist, cumul_dist_travelled, cumul_dist_included, inclusion_status

def draw_lines(cxcy, pts, frame, blur, mask):
    """
    This function draws a line between consecutive locations the object moves
     
    Parameters
    ----------
    cxcy: array, float
        The object's current location (its centroid) in the current frame
    pts: int
...
def HUD_info(frame, blur, mask, eq, line, columnID, frame_pos, cumul_dist_travelled, inclusion_status):
    """
    ...
    for img in (frame, blur, mask):
        ...
        # Display inclusion status in the HUD
        if inclusion_status:
            text_color = (0, 0, 255) if inclusion_status == "Excluded" else (0, 255, 0)
            cv2.putText(frame, f'Inclusion Status: {inclusion_status}', (15, 310), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color)
    
    return frame, blur, mask, eq

def output(frame, mask, eq, blur, mode="frame", inclusion_status=None, pause_on_excluded=False):
    """
    ...
    mode: str
        ...
        inclusion_status: str or None
        Inclusion status for the frame ("Included" or "Excluded"). If None, no inclusion status is displayed.
    pause_on_excluded: bool
        If True, the function will pause for one second on excluded frames.
    """
    if mode == "frame":
    ...
    if pause_on_excluded and inclusion_status == "Excluded":
        cv2.waitKey(100000) 


def frame_display_time(fps, mode = "slow_speed"):
    """
    ...
    extremelyfast_speed = int((1000/fps)/50) 
    ...
    if mode == "extremelyfast_speed":
        key = cv2.waitKey(extremelyfast_speed) & 0xFF
    if mode == "fast_speed":
